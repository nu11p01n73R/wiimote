...
class Wiimote(threading.Thread):
	state = None
	running = False
	ButtonState = BUTTONSTATE #empty values
	IRState = IRSTATE
	LEDState = LEDSTATE
	Battery = None
	...
	def SetLEDs(self, led1,led2,led3,led4):
		self.LEDState.LED1 = led1
		self.LEDState.LED2 = led2
		self.LEDState.LED3 = led3
		self.LEDState.LED4 = led4
		
		self._send_data((0x52,0x11,self.setter.SetLEDs(self.LEDState)))
	...
	def run(self):
		...
		while self.running:
			...
			if len(x) >= 4: self.parser.parseButtons((x[2]<<8) + x[3], self.ButtonState)
			if len(x) >= 19: self.parser.parseIR(x[7:19],self.IRState)
...
if __name__ == "__main__":
	...
	while 1:
		time.sleep(0.1)
		#print wiimote.state
		print wiimote.ButtonState.A, wiimote.ButtonState.B, wiimote.ButtonState.Up, wiimote.ButtonState.Down, wiimote.ButtonState.Left, wiimote.ButtonState.Right, wiimote.ButtonState.Minus, wiimote.ButtonState.Plus, wiimote.ButtonState.Home, wiimote.ButtonState.One, wiimote.ButtonState.Two, wiimote.IRState.RawX1, wiimote.IRState.RawY1, wiimote.IRState.Size1, wiimote.IRState.RawX2, wiimote.IRState.RawY2, wiimote.IRState.Size2
